...
The SDK now caches `Bucket`, `Scope`, and `Collection` instances for better performance when an application repeatedly gets the same bucket / scope / collection.

* https://jira.issues.couchbase.com/browse/SCBC-520[SCBC-520]:
The `couchbase2://` protocol now supports SQL++ query index management with `cluster.queryIndexes` and `collection.queryIndexes`.
