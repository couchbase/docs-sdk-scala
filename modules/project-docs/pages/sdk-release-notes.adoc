= SDK Release Notes
:description: Release notes, brief installation instructions, and download archive for the Couchbase Scala Client.
...
Version {sdk_dot_minor} of the Scala SDK implements the {sdk_api} xref:compatibility.adoc#api-version[SDK API].
...
* The Scala SDK is tested against LTS versions of OpenJDK --
...
[#latest-release]
== Scala SDK 3.12 Releases

Version 3.12 of the Scala SDK implement the 3.9 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.

NOTE: From 3.9.0 on, all Couchbase JVM SDKs have an aligned version number to make it easier to users to track changes. 
So the version jumped from 1.8.x to 3.9.x.
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.12.3/com/couchbase/client/scala/index.html[API Reference] |
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.12.2/com/couchbase/client/scala/index.html[API Reference] |
...
* https://jira.issues.couchbase.com/browse/SCBC-365[SCBC-365]:
The SDK now caches `Bucket`, `Scope`, and `Collection` instances for better performance when an application repeatedly gets the same bucket, scope, or collection.

* https://jira.issues.couchbase.com/browse/SCBC-520[SCBC-520]:
The `couchbase2://` protocol now supports {sqlpp} query index management with `cluster.queryIndexes` and `collection.queryIndexes`.
...
* https://jira.issues.couchbase.com/browse/SCBC-519[SCBC-519]:
Fixed an issue where using the bucket management APIs with `couchbase2://` would throw `NullPointerException` if the bucket did not have a minimum durability level set.

=== Version 3.12.1 (08 July 2026)

https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.12.1/com/couchbase/client/scala/index.html[API Reference] |
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.12.0/com/couchbase/client/scala/index.html[API Reference] |
...
* https://jira.issues.couchbase.com/browse/SCBC-507[SCBC-507]:
Improved the performance of the Futures API.
...
+
TIP: Developers using the SDK's reactive transaction API may see an increase in the number of threads spawned by the SDK, because each operation is now executed on its own dedicated thread.
For high-concurrency transactions running on Java 21 and later, we recommend using the blocking API and virtual threads.
...
* https://jira.issues.couchbase.com/browse/SCBC-509[SCBC-509]:
`EventingFunctionManager.functionsStatus()` no longer throws an exception if a function is in the `"resuming"` state.
The `EventingFunctionStatus` enumeration has two new case objects: `Resuming` and `Unknown` (a special value indicating the SDK did not recognize the status code returned by the server).

* https://jira.issues.couchbase.com/browse/SCBC-510[SCBC-510]:
`EventingFunctionManager.functionsStatus()` now correctly reports the status of a function in the `"undeploying"` state as `Undeploying` instead of incorrectly reporting it as `Deploying`.
...
== Scala SDK 3.11 Releases

=== Version 3.11.3 (21 May 2026)

https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.11.3/com/couchbase/client/scala/index.html[API Reference] |
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.11.2/com/couchbase/client/scala/index.html[API Reference] |
...
Alternatively, you can opt in via the `observabilitySemanticConventions` client setting, which has a higher precedence than the environment variable.
...
== Scala SDK 3.10 Releases

=== Version 3.10.1 (14 January 2026)

https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.10.1/com/couchbase/client/scala/index.html[API Reference] |
...
* https://jira.issues.couchbase.com/browse/SCBC-500[SCBC-500]: Deprecated the Views API.
+
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.10.0/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-496[SBC-496]:
Support mTLS Certs Refresh (without restart).
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.9.2/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-495[SCBC-495]:
`cluster.disconnect()` now cleanly shuts down, including waiting for the thread pool to exit.
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-3.9.1/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-492[SCBC-492]:
Fixed a regression that caused `getAllReplica` and `lookupInAllReplica` operations to fail unless explicitly passed a timeout.
...
Fixed a regression in 1.6.2 where attempting to manage unscoped (cluster-level) eventing functions throws a `NullPointerException`.
...
The goal of this alignment is twofold: to minimize confusion about which versions are compatible with each other, and to enable a more disciplined branch management strategy where patch releases contain only low-risk bug fixes.
...
It closes a connection if there is no incoming traffic for the duration specified by `io.configIdleRedialTimeout` (default value: 5 minutes), and sends a `NOOP` request if there is no incoming traffic for half that duration.
...
Improved the backpressure implementation for reactive {sqlpp}, Analytics, and Full-Text Search queries.
...
It specifies compatible versions of the Java, Scala, and Kotlin clients, as well as the optional metrics and tracing components. https://central.sonatype.com/artifact/com.couchbase.client/couchbase-client-bom[https://central.sonatype.com/artifact/com.couchbase.client/couchbase-client-bom].


== Scala SDK 1.8 Releases

Version 1.8 of the Scala SDK implements the 3.7 xref:compatibility.adoc#api-version[SDK API].

=== Version 1.8.3 (11 July 2025)

Version 1.8.3 is the third maintenance release of the 1.8 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.8.3/com/couchbase/client/scala/index.html[API Reference] |
...
=== Version 1.8.2 (4 June 2025)

Version 1.8.2 is the second maintenance release of the 1.8 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.8.2/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-487[SCBC-487]:
The `flexIndex` parameter in `TransactionQueryOptions` is now correctly passed to the server.

==== Improvements

* https://couchbasecloud.atlassian.net/browse/SCBC-486[SCBC-486]:
Case class constructors made public again, for all input case classes that are part of the public interface.
...
=== Version 1.8.1 (9 May 2025)

Version 1.8.1 is the first maintenance release of the 1.8 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.8.1/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-485[SCBC-485]:
`SearchQuery.toString()` has not been returning the query JSON since Scala SDK 1.4.5.
...
=== Version 1.8.0 (9 April 2025)

Version 1.8.0 is the first release of the 1.8 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.8.0/com/couchbase/client/scala/index.html[API Reference] |
...
* https://couchbasecloud.atlassian.net/browse/SCBC-459[SCBC-459]:
...
Version 1.7 of the Scala SDK implements the 3.6 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.


=== Version 1.7.9 (11 March 2025)

Version 1.7.9 is the ninth maintenance release of the 1.7 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.9/com/couchbase/client/scala/index.html[API Reference] |
...
=== Version 1.7.8 (11 February 2025)

Version 1.7.8 is the eighth maintenance release of the 1.7 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.8/com/couchbase/client/scala/index.html[API Reference] |
...
=== Version 1.7.7 (08 January 2025)

Version 1.7.7 is the seventh maintenance release of the 1.7 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.7/com/couchbase/client/scala/index.html[API Reference] |
...
* https://jira.issues.couchbase.com/browse/SCBC-474[SCBC-474]:
Fixed a regression introduced in Scala SDK 1.4.3 that caused `JsonArray` and `JsonObject` to no longer be handled correctly when used as parameters in `QueryOptions`, and in `SearchOptions.raw`.
...
=== Version 1.7.6 (04 December 2024)

Version 1.7.6 is the sixth maintenance release of the 1.7 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.6/com/couchbase/client/scala/index.html[API Reference] |
...
* https://jira.issues.couchbase.com/browse/SCBC-471[SCBC-471]:
Updated dependencies.


=== Version 1.7.5 (06 November 2024)

Version 1.7.5 is the fifth maintenance release of the 1.7 series.

https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.5/com/couchbase/client/scala/index.html[API Reference] |
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.3/com/couchbase/client/scala/index.html[API Reference] |
...
https://docs.couchbase.com/sdk-api/couchbase-scala-client-1.7.0/com/couchbase/client/scala/index.html[API Reference] |
...
Version 1.6 of the Scala SDK implements the 3.5 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.
...
and includes support for the `couchbase2://` connection protocol --
...
Version 1.3 of the Scala SDK implements the 3.3 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.
...
Version 1.2 of the Scala SDK implements the 3.2 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.
...
Version 1.1 of the Scala SDK implements the 3.1 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.
...
Version 1.0 of the Scala SDK implements the 3.0 xref:compatibility.adoc#api-version[SDK API].
See the xref:compatibility.adoc#couchbase-feature-availability-matrix[compatibility pages] for more information on feature compatibility with different versions of Couchbase Server.
