...
The Couchbase Scala SDK {sdk_dot_minor} Client supports Scala 2.12, 2.13, and 3.3 through 3.7 (inclusive).
...
We recommend running the latest LTS version of the JDK (at the time of writing, JDK {java_latest_lts_version}).
...
Note that from 3.9.0 on, all Couchbase JVM SDKs have an aligned version number to make it easier to users to track changes.
...
| Vector Search
| N/A
2+| From SDK 1.6.0 (base64 encoded vectors from 1.7.0)

| Preferred Server Group Replica Reads
| N/A
2+| From SDK 1.8.0 (with Server 7.6.2 onwards)
...
https://projects.spring.io/spring-data-couchbase/[Spring Data Couchbase^] uses the Java SDK underneath and as a result it also provides different compatibilities with Couchbase Server.
