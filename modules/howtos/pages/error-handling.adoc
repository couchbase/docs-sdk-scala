...
[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=key-not-found,indent=0]
----

[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=get,indent=0]
----
...
[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=key-exists,indent=0]
----
...
If it has, it returns `CasMismatchException`, and the most appropriate response is to simply retry it:

[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=cas,indent=0]
----
...
Another ambiguous error is `DurabilityAmbiguousException`, which can returned when performing a durable operation.
...
[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=insert,indent=0]
----
...
[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=insert-real,indent=0]
----
...
If that best effort fails, and the `doInsert` call still returns a `Failure`, then it's highly context-dependent how to handle that.
...
for inserts, we confirm if the operation has already been successful on an ambiguous result by checking for `DocumentExistsException`
...
[source,scala]
----
include::devguide:example$scala/ErrorHandling.scala[tag=query,indent=0]
----
