...
* Speed.  Our internal benchmarking indicates JsonObject is 20-30 times faster than the nearest Scala JSON library on some operations.  It achieves this mostly by being built around simple, but very fast, mutable JVM data structures.
* Ease-of-use.   We find ourselves in agreement with http://www.lihaoyi.com/post/uJsonfastflexibleandintuitiveJSONforScala.html[the author of µJson] that immutability provides a lot of complexity for dealing with deeply nested JSON, for very little gain, as JSON is most often dealt with in a limited scope (e.g. getting and modifying a document).

Of course, if you'd rather have an immutable JSON library with immutable
...
As with the other supported JSON libraries, JsonObject (and JsonArray) can easily be
...
=== Error Handling
The majority of the Scala SDK will not throw exceptions.  Methods on `JsonObject` are, well, an exception to this general rule.  If a field does not exist a `NoSuchElementException` will be thrown.
...
or use the support for `Dynamic`:
...
== Case Classes
It can be very useful to deal directly with Scala case classes, that is to send and retrieve them directly, and the Scala SDK includes built-in support for this.
