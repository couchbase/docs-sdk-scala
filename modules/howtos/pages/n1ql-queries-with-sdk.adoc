...
The Scala SDK supports returning N1QL results directly as a Scala case class.
...
available on the xref:kv-operations.adoc[Key-Value Operations] page, but the short version is to add a `Codec` in the case class's companion object like this:

[source,scala]
----
include::example$Queries.scala[tag=codec]
----
...
[source,scala]
----
include::example$Queries.scala[tag=case-classes]
----
