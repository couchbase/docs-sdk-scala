...
For complex and long-running queries, involving large ad hoc join, set, aggregation, and grouping operations, Couchbase Data Platform offers the xref:6.6@server:analytics:introduction.adoc[Couchbase Analytics Service (CBAS)].
...
Here we're fetching rows converted into `JsonObject`, but as with N1QL there's many more options available.
