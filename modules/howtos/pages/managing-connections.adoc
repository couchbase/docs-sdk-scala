...
Couchbase's next generation connection protocol, introduced in Scala SDK 1.5 and a future release of Couchbase Autonomous Operator, can be enabled simply by changing the connection string to `couchbase2://` but there are a few differences to be aware of, described <<#limitations,below>>.
