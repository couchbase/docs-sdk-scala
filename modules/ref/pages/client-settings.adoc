...
// Kotlin - add https://www.couchbase.com/forums/t/max-idle-connections-error/38183
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=create-env-simple,indent=0]
----
...
Timeout settings are configured using an instance of `TimeoutConfig`.
As with most options blocks in the SDK, this is a Scala case class - e.g. an immutable data class - with fluent-style methods that return new immutable copies with a new setting applied.

.Creating a new timeout config builder
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=timeout,indent=0]
----
...
Many client settings may also be configured by setting a Scala system property.
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=system-properties,indent=0]
----
...
TIP: System property names starting with `com.couchbase.env` are paths in an object graph rooted at the environment builder.
Setting the property `com.couchbase.env.timeout.kvTimeout` maps to `TimeoutConfig.kvTimeout` (the 'Config' is implicit and should not be included in the property).
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=security,indent=0]
----
...
I/O settings are represented by the Scala class `IoConfig`.
The associated `ClusterEnvironment.Builder` method is called `ioConfig`.

.Template for configuring I/O settings
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=io-config,indent=0]
----
...
+
Mutation tokens allow enhanced durability requirements as well as advanced querying capabilities.
Set this to `false` if you do not require these features and wish to avoid the very small associated overhead.
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=circuit-breaker,indent=0]
----
...
Timeout settings are represented by the Scala class `TimeoutConfig`.
...
Compression settings are represented by the Scala class `CompressionConfig`.
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=compression,indent=0]
----
...
[source,scala]
----
include::devguide:example$scala/ClientSettings.scala[tag=general,indent=0]
----
...
A `ClusterEnvironment.WanDevelopmentProfile` configuration profile can be used to modify client settings for development or high-latency environments.
