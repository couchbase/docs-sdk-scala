...
(There's a *View* link to the complete sample code on GitHub above each of the snippets on the page, and a *Copy* icon to grab just the snippet shown.)
...
xref:howtos:json.adoc#error-handling-and-jsonobjectsafe[here] for details),
...
// The Student Records Tutorial gives an introduction to data modeling in a document database alongside using the {name-sdk}.
...
* Discover xref:concept-docs:querying-your-data.adoc[SQL++] -- our SQL-family querying language.
