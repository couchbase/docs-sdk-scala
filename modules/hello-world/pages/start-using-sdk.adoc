...
The Couchbase Scala SDK is available on the Maven repository, with published builds for Scala 2.12 and 2.13.
The latest version, as of June 2021, is 1.1.6.
...
----
dependencies {
    compile group: 'com.couchbase.client', name: 'scala-client_2.12', version: '1.1.6'
}
----
...
----
dependencies {
    compile group: 'com.couchbase.client', name: 'scala-client_2.13', version: '1.1.6'
}
----
...
+
--
You connect to a Couchbase Cloud cluster the same as any other cluster, except that the use of TLS is mandatory.
...
It is possible to connect using TLS without a server certificate using this alternative `ClusterEnvironment` configuration:

[source,scala]
----
include::example$Cluster.scala[tag=cloud-cluster-insecure,indent=0]
----
This is not a secure connection.
It is only suitable for debugging purposes and should not be used in production.
--
====
...
Couchbase Server, since version 7.0, supports scopes and collections, allowing documents to be grouped granually.
Here we will just open the 'default collection', which includes all documents in a bucket, and is forwards and backwards compatible with all supported versions of Couchbase Server.
