...
* Capella Columnar (currently in private preview) for real-time analytics, with data stored elsewhere in your data lake.
* Couchbase Analytics Service (CBAS) for traditional data analytics.

Both use the {sqlpp} query language.
...
or dive in and try it xref:howtos:vector-searching-with-sdk.adoc[with the {name-sdk}].
...
Creating the right index, with the right keys, right order, and right expression is critical to query
performance in any database system. That’s true for Couchbase as well.
////
